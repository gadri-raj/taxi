...
import 'package:raj_e_commerce_flutter/models/Address.dart';
...
class UserDatabaseHelper {
  ...
  static const String ORDERED_PRODUCTS_COLLECTION_NAME = "bookings";
  ...
  Future<bool> update(String cartItemID) async {
    String uid = AuthentificationService().currentUser.uid;
    final cartCollectionRef = firestore
        .collection(ORDERED_PRODUCTS_COLLECTION_NAME);
    final docRef = cartCollectionRef.doc(cartItemID);
    final docSnapshot = await docRef.get();
    String currentCount = docSnapshot.data()[OrderedProduct.Status_KEY];
    if (currentCount == "pending") {
      docRef.update({OrderedProduct.Status_KEY: "cancelled"});
    }
    return true;
  }

  Future<bool> updateAdmin(String cartItemID) async {
    String uid = AuthentificationService().currentUser.uid;
    final cartCollectionRef = firestore
        .collection(ORDERED_PRODUCTS_COLLECTION_NAME);
    final docRef = cartCollectionRef.doc(cartItemID);
    ...
    String currentCount = docSnapshot.data()[OrderedProduct.Status_KEY];
    if (currentCount == "pending") {
      docRef.update({OrderedProduct.Status_KEY: "accepted"});
    }
    return true;
  }
  ...
  Future<List<String>> get adminorderedProductsList async {
    String uid = AuthentificationService().currentUser.uid;
  ...
  Future<List<String>> get orderedProductsList async {
    String uid = AuthentificationService().currentUser.uid;
    final orderedProductsSnapshot = await firestore
        .collection(ORDERED_PRODUCTS_COLLECTION_NAME).where('uid', isEqualTo: uid)
        .get();
    List orderedProductsId = List<String>();
    for (final doc in orderedProductsSnapshot.docs) {
      orderedProductsId.add(doc.id);
  ...
  Future<bool> addToMyOrders(List<OrderedProduct> orders) async {
    String uid = AuthentificationService().currentUser.uid;
    final orderedProductsCollectionRefer = firestore
        .collection(ORDERED_PRODUCTS_COLLECTION_NAME);
    for (final order in orders) {
      await orderedProductsCollectionRefer.add(order.toMap());
    }
    return true;
  }

  Future<OrderedProduct> getadminOrderedProductFromId(String id) async {
    String uid = AuthentificationService().currentUser.uid;
    final doc = await firestore
        .collection(ORDERED_PRODUCTS_COLLECTION_NAME)
        .doc(id)
